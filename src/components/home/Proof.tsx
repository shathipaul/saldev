...
import { CASE_STUDIES, TESTIMONIALS, type CaseStudy, type Testimonial } from "@/data/proof";
import { cn } from "@/lib/cn";

/** Marks sample copy that must be replaced before launch (orange highlight). */
function Sample({ children, big = false }: { children: ReactNode; big?: boolean }) {
  return (
    <span
      className={cn(
...
function CaseStudyCard({ study }: { study: CaseStudy }) {
  const identity = (text: string) => (study.sampleIdentity ? <Sample>{text}</Sample> : text);
    ...
    <article className="cs rv relative overflow-hidden rounded-lg border border-line bg-white px-7 pt-7 pb-[26px] transition-[transform,box-shadow,border-color] duration-[450ms] ease-brand hover:-translate-y-1 hover:border-orange-pale hover:shadow-[0_24px_48px_-20px_rgba(255,94,0,.3)]">
      <div className="relative mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-[12.8px] text-ink-5">{identity(study.tagline)}</p>
        </div>
      </div>
      ...
      <div className="relative mt-4 flex items-center gap-[7px] border-t border-line pt-3.5 text-[13.2px] text-ink-4">
        <CheckIcon className="size-3.5 shrink-0 text-orange" strokeWidth={2.4} />
...
function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <figure className="relative rounded-lg border border-line bg-white px-7 py-[30px] transition-[transform,box-shadow,border-color] duration-[450ms] ease-brand hover:-translate-y-1 hover:border-orange-pale hover:shadow-[0_22px_44px_-20px_rgba(255,94,0,.28)]">
      <div className="mb-1.5 text-[44px] leading-none font-bold text-orange-pale" aria-hidden="true">
        “
      </div>
      ...
      <figcaption className="flex items-center gap-3">
        ...
        />
        <span>
          <span className="text-[14.5px] font-semi text-ink">{testimonial.name}</span>
          <br />
    ...
    <section id="proof" className="relative z-[1] py-24 max-cols:py-[70px]">
      <div className="mx-auto w-full max-w-page px-6">
        <SectionIntro
          ...
          lede={
            <>
              <b className="font-semibold text-ink">Two quotes in, case studies still to come.</b>{" "}
              Nothing gets invented on this page, so the numbers stay blank until you send them.
            </>
          }
        />
        ...
        <div className="grid gap-[18px]">
          <p className={cn("rv", NOTICE)}>
            ...
            Sample figures for layout only. Everything in orange is placeholder text, not a real
            result. Replace before this page goes anywhere near a prospect.
          </p>
